...
def parse_args():
   ...
   epilog='-----------------------------------------------------------------------------------\n'
          ...
          '      python login_pharmer.py -s "dd-wrt" -api Wutc4c3T78gRIKeuLZesI8Mx2ddOiP4\n\n'
          '  -Open a range of IP addresses, print the title of each response page and\n'
          '   make 1000 requests concurrently:\n\n'
          '      python login_pharmer.py -t 192.168.0-5.1-254 -c 1000\n\n'
          ...
          '      python login_pharmer.py -s "dd-wrt" -u root -p admin -f ">Advanced Routing<"\n\n'
          '  -Open www.reddit.com specifically with https:// and attempt to login using "sirsmit418"\n'
          '   and password "whoopwhoop":\n\n'
          '      python login_pharmer.py -t www.reddit.com -ssl -u sirsmit418 -p whoopwhoop')

   parser.add_argument("-s", "--shodansearch", help="Your search terms")
   parser.add_argument("-c", "--concurrent", default='500', help="Enter number of requests to make concurrent; default = 500")
        ...
        try:
            ...
            if mark == '*' or mark == '+':
                with open('%s_results.txt' % self.search, 'a') as f:
                    f.write('[%s] %s | %s' % (mark, target, label))
            print results
        except Exception as e:
...
def main(args):
    ...
    if args.targets:
        ...
        targets = shodan_search(args.shodansearch, args.apikey)
    ...
    # By default run 500 concurrently at a time
